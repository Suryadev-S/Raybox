...
const RayboxHome = async ({ searchParams }: {
  ...
  const currentPage = parseInt(queries.page as string | undefined || '1');
  const limit = parseInt(queries.limit as string | undefined || '25');
  ...
  const nextPageUrl = `/raybox/${session.user.name}?${nextPageParams}`;
  const previousPageUrl = `/raybox/${session.user.name}?${previousPageParams}`;
        ...
        <PaginationContent>
          ...
          <PaginationItem>
            <Button size={'sm'} variant={'outline'}>
              {resource.meta.currentPage}
            </Button>
          </PaginationItem>
          <PaginationItem>of</PaginationItem>
          <PaginationItem>
            <Button size={'sm'} variant={'outline'}>
              {resource.meta.totalPages}
            </Button>
          </PaginationItem>
          {resource.meta.currentPage !== resource.meta.totalPages && (
            <PaginationItem>
