...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
    try {
        ...
        const filePathAbsolute: string = path.join(process.env.STORAGE_PREFIX as string, 'media', fileType, hashPartitions, fileName);
        const filePathRelative: string = path.join('media', fileType, hashPartitions, fileName);
